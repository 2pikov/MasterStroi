            ...
            <div class="product__meta">
                <div class="product__meta-left">
                    ...
                    <div class="product__rating">
                        ...
                        <a href="#reviews" class="reviews-link">{{ $product->reviews->where('is_approved', true)->count() }} отзывов</a>
                    </div>
        ...
        if (reviewForm) {
            reviewForm.addEventListener('submit', function(e) {
                ...
                .then(response => {
                    ...
                    if (response.status === 403) {
                        response.json().then(data => {
                            showToast(data.message || 'Вы не можете оставить отзыв на этот товар', 'error');
                        });
                        return;
                    }
                    return response.json();
                ...
                .then(data => {
                    if (data && data.success) {
                        reviewForm.reset();
                        showToast('Ваш отзыв отправлен на модерацию и появится после одобрения администратором', 'success');
                    } else {
                        if (data.errors) {
